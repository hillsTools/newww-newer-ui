...
-- No exploits; purely UI. Works in LocalScript (StarterPlayerScripts) or Plugin testing.
-- Style inspired by “neon blue panel” look, condensed for phones.
...
function MiniUI:CreateWindow(opts)
    ...
    local gui = mk("ScreenGui", {Name = opts.name or "MiniUI", ResetOnSpawn=false, IgnoreGuiInset=true})
    ...
    if draggable then
        ...
        root.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
                dragging = true; offset = i.Position - root.AbsolutePosition
            end
        end)
        ...
        game:GetService("UserInputService").InputChanged:Connect(function(i)
            if dragging and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
                root.Position = UDim2.fromOffset(i.Position.X - offset.X, i.Position.Y - offset.Y)
            end
        end)
    end

    -- Top bar
    local top = mk("Frame", {Parent=root, BackgroundTransparency=1, Size=UDim2.new(1,-16,0,36), Position=UDim2.fromOffset(8,6)})
    local titleLbl = text(top, title, 18, true); titleLbl.Size = UDim2.new(1,0,1,0); titleLbl.TextXAlignment=Enum.TextXAlignment.Left

    -- Tabs row
    local tabBar = mk("Frame", {Parent=root, BackgroundTransparency=1, Size=UDim2.new(1,-16,0,28), Position=UDim2.fromOffset(8,44)})
    local tabLayout = mk("UIListLayout", {Parent=tabBar, FillDirection=Enum.FillDirection.Horizontal, Padding=UDim.new(0,14), VerticalAlignment=Enum.VerticalAlignment.Center})

    -- Content host
    local content = mk("Frame", {Parent=root, BackgroundTransparency=1, Size=UDim2.new(1,-16,1,-90), Position=UDim2.fromOffset(8,76)})

    -- Public window object
    local win = { _gui=gui, _root=root, _tabBar=tabBar, _content=content, _tabs={}, _active=nil }

    function win:AddTab(name)
        local btn = mk("TextButton", {
            Parent=tabBar, AutoButtonColor=false, BackgroundTransparency=1, Text=name,
            FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold),
            TextColor3 = THEME.text, TextSize=18
        })
        btn.Size = UDim2.fromOffset(#name*9+14, 28)

        local page = mk("Frame", {Parent=content, BackgroundTransparency=1, Size=UDim2.new(1,0,1,0), Visible=false})

        local function activate()
            for _,t in pairs(win._tabs) do t.page.Visible=false; t.btn.TextColor3=THEME.text end
            page.Visible=true; btn.TextColor3=THEME.accent; win._active = page
        end
        ...
        local scroller = mk("ScrollingFrame", {
            Parent = page, BackgroundTransparency=1, Size=UDim2.fromScale(1,1),
            CanvasSize=UDim2.new(0,0,0,0), ScrollBarThickness=6, AutomaticCanvasSize=Enum.AutomaticSize.Y
        })
        local list = mk("UIListLayout", {Parent=scroller, Padding=UDim.new(0,10)})
        list.SortOrder = Enum.SortOrder.LayoutOrder

        function tabObj:AddSection(headerText)
            local holder = mk("Frame", {Parent=scroller, BackgroundTransparency=1, Size=UDim2.new(1,0,0,0), AutomaticSize=Enum.AutomaticSize.Y})
            local header = text(holder, headerText, 18, true, THEME.accent, true)
            header.Size = UDim2.new(1,0,0,26)

            local panel = mk("Frame", {Parent=holder, BackgroundColor3=THEME.panel2, Size=UDim2.new(1,0,0,0), AutomaticSize=Enum.AutomaticSize.Y})
            round(panel, 8); stroke(panel, 1, THEME.border, 0.45)

            local innerScroll = mk("ScrollingFrame", {
                Parent=panel, BackgroundColor3=THEME.panel, BorderSizePixel=0,
                Position=UDim2.fromOffset(8,8), Size=UDim2.new(1,-16,0,0), AutomaticSize=Enum.AutomaticSize.Y,
                CanvasSize=UDim2.fromScale(0,0), ScrollBarThickness=6
            })
            round(innerScroll, 6); stroke(innerScroll, 1, THEME.border, 0.6)

            local lay = mk("UIListLayout", {Parent=innerScroll, Padding=UDim.new(0,8)})
            ...
            local function rowBase(h)
                local r = mk("Frame", {Parent=innerScroll, BackgroundColor3=THEME.panel, Size=UDim2.new(1,-16,0,h or 36)})
            ...
            function sec:AddButton(labelText, callback)
                ...
                local btn = mk("TextButton", {Parent=r, BackgroundColor3=THEME.panel2, AutoButtonColor=false, Text="Click", TextColor3=THEME.text, TextSize=16, Size=UDim2.fromOffset(64,26), Position=UDim2.new(1,-76,0.5,-13)})
            ...
            function sec:AddToggle(labelText, default, callback)
                local r = rowBase()
                local lbl = text(r, labelText, 16, false); lbl.Size=UDim2.new(1,-110,1,0); lbl.Position=UDim2.fromOffset(12,0)
                local btn = mk("TextButton", {Parent=r, BackgroundColor3= default and THEME.on or THEME.off, AutoButtonColor=false, Text= default and "On" or "Off", TextColor3=THEME.text, TextSize=16, Size=UDim2.fromOffset(64,26), Position=UDim2.new(1,-76,0.5,-13)})
                round(btn,6); stroke(btn,1,THEME.border,0.5)
            ...
            function sec:AddNumberBox(labelText, default, callback)
                ...
                local box = mk("TextBox", {Parent=r, BackgroundColor3=THEME.panel2, Text=tostring(default or 0), TextSize=16, TextColor3=THEME.text, ClearTextOnFocus=false, Size=UDim2.fromOffset(80,26), Position=UDim2.new(1,-92,0.5,-13)})
            ...
            function sec:AddSlider(labelText, min, max, default, callback)
                ...
                local bar = mk("Frame", {Parent=r, BackgroundColor3=THEME.panel2, Size=UDim2.new(1,-32,0,8), Position=UDim2.fromOffset(12,28)})
                round(bar,4); stroke(bar,1,THEME.border,0.6)
                local fill = mk("Frame", {Parent=bar, BackgroundColor3=THEME.accent, Size=UDim2.new((default-min)/(max-min),0,1,0)})
                round(fill,4)
            ...
            function sec:AddDropdown(labelText, options, defaultIndex, callback)
                ...
                local dd = mk("TextButton", {Parent=r, BackgroundColor3=THEME.panel2, AutoButtonColor=false, Text=current, TextColor3=THEME.text, TextSize=16, Size=UDim2.fromOffset(140,26), Position=UDim2.new(1,-156,0.5,-13)})
                round(dd,6); stroke(dd,1,THEME.border,0.5)

                local listOpen = mk("Frame", {Parent=r, BackgroundColor3=THEME.panel2, Visible=false, Position=UDim2.new(1,-156,0,34), Size=UDim2.fromOffset(140,0), AutomaticSize=Enum.AutomaticSize.Y})
                round(listOpen,8); stroke(listOpen,1,THEME.border,0.5)
                local s = mk("ScrollingFrame", {Parent=listOpen, BackgroundTransparency=1, CanvasSize=UDim2.fromScale(0,0), AutomaticCanvasSize=Enum.AutomaticSize.Y, Size=UDim2.new(1,0,1,0), ScrollBarThickness=6})
                local l = mk("UIListLayout", {Parent=s, Padding=UDim.new(0,4)})

                local function setChoice(v)
                    dd.Text = v; if callback then callback(v) end
                end
                for _,opt in ipairs(options) do
                    local b = mk("TextButton", {Parent=s, AutoButtonColor=true, BackgroundColor3=THEME.panel, Text=opt, TextColor3=THEME.text, TextSize=16, Size=UDim2.new(1,-8,0,28)})
